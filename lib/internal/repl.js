...
module.exports.createInternalRepl = createRepl; // XXX(chrisdickinson): The 15ms debounce value is somewhat arbitrary.
...
function createRepl(env, opts, cb) {
  ...
  if (opts.terminal) {
    return setupHistory(repl, env.NODE_REPL_HISTORY, env.NODE_REPL_HISTORY_FILE, cb);
  }
...
function setupHistory(repl, historyPath, oldHistoryPath, ready) {
  ...
  if (typeof historyPath === 'string') historyPath = historyPath.trim();
  ...
  var writing = false;
  var pending = false;
  repl.pause(); // History files are conventionally not readable by others:
  ...
  function onread(err, data) {
    ...
    if (data) {
      ...
      _writeToOutput(repl, '\nThe old repl history file has the same name and location as ' + "the new one i.e., ".concat(historyPath, " and is empty.\nUsing it as is.\n"));
      ...
      try {
        ...
        var oldReplJSONHistory = fs.readFileSync(oldHistoryPath, 'utf8'); // Only attempt to use the history if there was any.
        ...
        // Don't crash, just don't persist history.
        threw = true;
        var type = err instanceof SyntaxError ? 'parse' : 'open';

        _writeToOutput(repl, "\nError: Could not ".concat(type, " old history file.\n") + 'REPL session history will not be persisted.\n');
      }

      if (!threw) {
        // Grab data from the older pre-v3.0 JSON NODE_REPL_HISTORY_FILE format.
        _writeToOutput(repl, '\nConverted old JSON repl history to line-separated history.\n' + "The new repl history file can be found at ".concat(historyPath, ".\n"));
      } else {
        repl.history = [];
      }
    }
  ...
  function onhandle(err, hnd) {
    ...
    fs.ftruncate(hnd, 0, function (err) {
      repl._historyHandle = hnd;
      repl.on('line', online); // reading the file data out erases it
